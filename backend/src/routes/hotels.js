...
async function aGet(path, params = {}) {
  try {
    ...
    if (error.response?.status === 401) {
      // Token might be expired, force refresh next time
      token = null;
      tokenExpiry = 0;
    }
    throw error;
...
router.get('/search', async (req, res) => {
  try {
    ...
    try {
      ...
      console.error('❌ Amadeus API error:', amadeusError.response?.data || amadeusError.message);
      
      // Return proper error response instead of mock data
      const statusCode = amadeusError.response?.status || 503;
      const errorMessage = amadeusError.response?.data?.error_description || amadeusError.message || 'Hotel search service temporarily unavailable';
