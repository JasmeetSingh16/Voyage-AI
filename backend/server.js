...
async function getToken() {
  ...
  try {
    ...
    throw new Error(`Amadeus API authentication failed: ${error.response?.data?.error_description || error.message}`);
  }
}
...
function generateMockHotels(cityCode, checkInDate, checkOutDate, adults) {
  const cityNames = {
    DEL: "New Delhi", BOM: "Mumbai", GOI: "Goa", BLR: "Bangalore", 
    PAR: "Paris", LON: "London", NYC: "New York", DXB: "Dubai",
    ATQ: "Amritsar", MAA: "Chennai", HYD: "Hyderabad", CCU: "Kolkata"
  };
  ...
  const mockHotels = [
    {
      ...
      hotel: {
        ...
        address: {
          cityName: cityName,
          countryCode: cityCode.length === 3 ? "IN" : "US",
          lines: [`123 Main Street, ${cityName}`]
        }
      },
      offers: [{
        id: `MOCK${cityCode}001-OFFER1`,
        checkInDate,
        checkOutDate,
        roomQuantity: 1,
        price: { total: basePrice + 120, currency: "USD" },
        policies: { cancellation: { type: "FULL_REFUND" } },
        room: { type: "DELUXE", description: { text: "Deluxe Room with City View" } }
      }]
    },
    {
      type: "hotel-offers",
      hotel: {
        hotelId: `MOCK${cityCode}002`,
        name: `${cityName} Plaza`,
        rating: 4,
        address: {
          cityName: cityName,
          countryCode: cityCode.length === 3 ? "IN" : "US",
          lines: [`456 Central Avenue, ${cityName}`]
        }
      },
      offers: [{
        id: `MOCK${cityCode}002-OFFER1`,
        checkInDate,
        checkOutDate,
        roomQuantity: 1,
        price: { total: basePrice + 60, currency: "USD" },
        policies: { cancellation: { type: "FULL_REFUND" } },
        room: { type: "STANDARD", description: { text: "Standard Room" } }
      }]
    },
    {
      type: "hotel-offers",
      hotel: {
        hotelId: `MOCK${cityCode}003`,
        name: `${cityName} Inn & Suites`,
        rating: 3,
        address: {
          cityName: cityName,
          countryCode: cityCode.length === 3 ? "IN" : "US",
          lines: [`789 Park Road, ${cityName}`]
        }
      },
      offers: [{
        id: `MOCK${cityCode}003-OFFER1`,
        checkInDate,
        checkOutDate,
        roomQuantity: 1,
        price: { total: basePrice + 20, currency: "USD" },
        policies: { cancellation: { type: "NON_REFUNDABLE" } },
        room: { type: "ECONOMY", description: { text: "Economy Room" } }
      }]
    },
    {
      type: "hotel-offers",
      hotel: {
        hotelId: `MOCK${cityCode}004`,
        name: `Luxury ${cityName} Resort`,
        rating: 5,
        address: {
          cityName: cityName,
          countryCode: cityCode.length === 3 ? "IN" : "US",
          lines: [`321 Beach Front, ${cityName}`]
        }
      },
      offers: [{
        id: `MOCK${cityCode}004-OFFER1`,
        checkInDate,
        checkOutDate,
        roomQuantity: 1,
        price: { total: basePrice + 200, currency: "USD" },
        policies: { cancellation: { type: "FULL_REFUND" } },
        room: { type: "SUITE", description: { text: "Presidential Suite with Ocean View" } }
      }]
    },
    {
      type: "hotel-offers",
      hotel: {
        hotelId: `MOCK${cityCode}005`,
        name: `Budget ${cityName} Lodge`,
        rating: 2,
        address: {
          cityName: cityName,
          countryCode: cityCode.length === 3 ? "IN" : "US",
          lines: [`555 Budget Street, ${cityName}`]
        }
      },
      offers: [{
        id: `MOCK${cityCode}005-OFFER1`,
        checkInDate,
        checkOutDate,
        roomQuantity: 1,
        price: { total: basePrice - 20, currency: "USD" },
        policies: { cancellation: { type: "NON_REFUNDABLE" } },
        room: { type: "BASIC", description: { text: "Basic Room" } }
      }]
    }
  ];
  ...
  return {
    ...
    via: "mock-data",
    data: mockHotels,
    resolvedCity: cityCode,
    usedNearestDates: false,
    meta: { 
      message: "Using mock data - Amadeus API credentials may be invalid. Get new credentials at https://developers.amadeus.com",
      stage: ["mock-fallback"]
    }
  };
}
  ...
  try {
    ...
    if (!token) {
      return res.status(503).json({
        error: true,
        message: "Unable to authenticate with hotel service provider",
        service: "amadeus",
